...
- **평가판 안내**:
  - 정식 라이선스가 등록되지 않은 상태에서는 생성된 클립보드 이미지 및 슬라이드에 평가판 기본 워터마크가 자동 삽입됩니다.
- **정식 라이선스 인증**:
  - `설정 -> 라이선스 등록(L)` 메뉴에서 [내 PC 고유 식별자(HWID)]를 복사하여 라이선스를 발급받은 후, 시리얼 키를 입력하고 `인증하기` 버튼을 누르면 정식 버전으로 활성화됩니다.
- **4대 표준 라이선스 에디션**:
  - ① **기업용 (ENTERPRISE - `MSENT`)**: 일반 기업체, 금융사, **공기업, 준정부기관** 대상 볼륨/사이트 라이선스.
  - ② **관공서용 (GOVERNMENT - `MSGOV`)**: 중앙부처, 국회·법원, 광역·기초 지방자치단체 등 순수 행정기관 대상 라이선스.
  - ③ **교육용 (EDUCATION - `MSED`)**: 초·중·고교, 대학교, 대학원, 직업훈련학원 및 교사·학생 대상 특별 라이선스.
  - ④ **개인용 (PERSONAL - `MSPS`)**: 1인 개발자, 독립 프리랜서 대상 1카피 라이선스.
- **기업/교육/관공서 전용 커스텀 워터마크 지정 기능**:
  - `서식·설정` 탭의 `워터마크 설정` 또는 상단 메뉴 `설정 -> 워터마크 설정(W)`을 통해 호출합니다.
  - 출력 위치 7종(중앙 대각선, 중앙 수평, 우측 하단, 좌측 하단, 우측 상단, 좌측 상단, 바둑판 타일) 지원.
  - 글꼴 크기(12~120pt), 회전 각도(-90°~+90°), 투명도(5%~100%), 색상, 굵기를 자유롭게 조정하고 우측 캔버스에서 실시간으로 렌더링 결과를 즉시 확인할 수 있습니다.
