...
### 📁 통합 프로젝트 확장자 `.drg` 공식 적용
- **기본 프로젝트 확장자 전환**: 기존 `.dragon`에서 가볍고 표준적인 **`.drg`**로 전면 전환.
- **저장/자동저장/불러오기 지원**: `Manual_Project_001.drg`, `.autosave.drg` 기본 생성.
- **하위 호환성 100% 보장**: 기존에 생성된 `.dragon` 및 `.mcs.json` 파일도 열기/병합 다이얼로그에서 그대로 완벽 지원.
- **글로벌 13개 언어 동기화**: `i18n_manager.py`의 모든 언어 파일 필터 및 툴팁을 `*.drg` 기준으로 갱신.

### 🛡️ RSA-2048 비대칭키 라이선스 보안 고도화 & 판매 연동 엔진 탑재
- **RSA-2048 비대칭키 디지털 서명 도입**:
  - 비밀키(Private Key)는 사장님 PC(`tools/secret_private_key.pem`) 및 판매 서버에만 영구 보관 (외부 유출 0%).
  - 고객용 `ManualStudio.exe`에는 오직 수학적 검증용 공개키(`RSA_PUBLIC_N`, `RSA_PUBLIC_E`)만 탑재.
  - 역공학/디스어셈블리 공격을 당하더라도 키 발급기 복제나 위조가 수학적으로 100% 원천 불가능.
- **클라이언트 제로 디펜던시(Zero Dependency) 무결성**:
  - 외부 암호화 DLL 없이 Python 내장 `pow(sig, e, n)` 연산만으로 0.001초 만에 검증.
- **판매 사이트 결제 연동 웹훅 모듈 (`tools/license_api_handler.py`) 신설**:
  - 쇼핑몰(스마트스토어, 아임웹, 자사몰, 토스/포트원 웹훅 등)에서 결제 즉시 호출하여 정식 시리얼 키 자동 발급·전송 지원.
- **개인키 Git 보호**: `.gitignore`에 `*.pem` 및 발급 DB 등록으로 GitHub 원격 유출 차단.

## [v1.9.7] - 2026-09-17

### 📱 매뉴얼 스튜디오 모바일 사용자 편익 극대화 (UX/연결/정렬/전송 전면 혁신)
- **앱 실행 시 기본 다이어그램 제거**:
  - 최초 실행 시 불필요한 기본 노드 없이 **완전한 빈 캔버스**로 시작.
- **도형 추가 시 넉넉한 간격 스마트 자동 배치 (겹침 원천 차단)**:
  - 노드 추가 간격을 대폭 확장(수직 gap 42dp, 수평 gap 55dp)하여 샌드위치처럼 바짝 붙거나 겹치는 현상 완전 해결.
- **상하/좌우 합리적 마그넷 꼭지점 지능형 자동 판정**:
  - 두 노드의 상대적 기하학적 위치(상하/좌우/대각선)를 실시간 분석하여, 가장 자연스럽고 합리적인 접점 포트(상↔하 or 좌↔우)를 100% 자동 결정하여 연결.
- **재연결 시 기존 연결선 자동 정리 (단일 연결성 보장)**:
  - 이미 연결된 다이어그램 사이에 새 접점으로 다시 연결하면, 이전 연결선은 자동으로 삭제되고 마지막에 연결한 선 1개만 깔끔하게 유지.
- **다이어그램 및 연결선 즉시 선택 & 원클릭 삭제**:
  - 노드 탭 시 즉시 하이라이트 + 하단 바 `🗑️ 삭제` / `✏️ 수정` 버튼 표출.
  - 연결선 탭 시 주황색 하이라이트 + 즉시 `🗑️ 삭제` 버튼 표출로 간편하게 선 제거 가능.
- **노드 이동 시 연결선 실시간 마그넷 추종 완벽 복원**:
  - 노드를 드래그하여 이동할 때, 마그넷으로 연결된 연결선이 실시간으로 찰싹 달라붙어 부드럽게 따라오도록 반응형 상태 동기화.
- **⚡ 자동정렬 기능 정상 작동 및 계층형 트리 정렬**:
  - Compose 상태 불변 객체 갱신을 통해 자동정렬 버튼 클릭 즉시 화면 전체 노드가 수직(TD) / 수평(LR) 질서정연한 격자 배치로 즉시 전환.
- **PC 전송 흐름 혁신 (2트랙 분리: 📶 사내 Wi-Fi vs 🌐 원격/LTE 어디서나)**:
  - `[📶 사내 Wi-Fi]`: 동일 공유기 내 직접 초고속 LAN 전송 (IP + PIN 자동 기억).
  - `[🌐 원격/LTE 어디서나]`: 외부 LTE/5G 환경에서도 IP 입력 없이 PC 화면의 6자리 PIN 번호만으로 원격 릴레이 즉시 전송.

## [v1.9.6] - 2026-09-17

### 📱 매뉴얼 스튜디오 모바일 (APK) 대대적 개편
- **ISO 5807 표준 플로우차트 기호 완벽 렌더링**:
  - 시작/종료: 완전한 둥근 알약형 (Capsule / Stadium)
  - 일반 작업: 표준 직사각형 (Rectangle)
  - 조건 분기: **정통 마름모 / 다이아몬드 (Rhombus)**
  - 데이터 입출력: **15도 기울어진 평행사변형 (Parallelogram)**
  - 데이터베이스: **원통형 실린더 (Cylinder)**
  - 문서 서식: **하단 물결형 문서 (Document with wavy bottom)**
- **4개 마그넷 꼭지점(Anchor Ports: 상/하/좌/우) 및 연결선 기능 신규 탑재**:
  - 각 노드 둘레에 4개의 마그넷 접점 포트 배치.
  - 상단 앱바 `🔗` 연결 모드로 노드 간 마그넷 포트를 터치하여 커스텀 연결선(FlowEdge) 생성.
  - 노드 추가 시 직전 노드와 지능형 자동 연결선 생성.
  - 직각 꺾임선(Orthogonal) 및 화살표 머리 렌더링.
  - PC 전송 시 `ElbowArrowItem`으로 일체 전송되어 PC 캔버스에서도 연결선 100% 호환.
- **카메라 직접 촬영 연동**:
  - 파일 첨부기/선택기 우회, 안드로이드 전용 `MediaStore.ACTION_IMAGE_CAPTURE`로 순수 카메라 앱 직접 실행.
- **시스템 UI 겹침 방지 (Safe Area 완벽 확보)**:
  - `navigationBarsPadding()` 적용: 세로/가로 모드 회전 시 폰 홈 버튼/제스처 바와 버튼이 절대 겹치지 않도록 안전 영역 확보.
  - `statusBarsPadding()` 적용: 상단 상태바 겹침 방지.
- **화면 회전 시 초기화 방지 및 로컬 영속 자동 저장 (Persistence)**:
  - `SharedPreferences` 기반 JSON 자동 저장/복원: 화면 회전, 앱 전환, 재시작 시에도 작업 중이던 플로우차트와 연결선 100% 보존.
- **샌드위치 뭉침 방지 스마트 자동 배치 (Anti-collision Smart Placement)**:
  - 노드 추가 시 기존 노드와 겹치지 않는 빈 영역을 계산하여 질서정연하게 자동 배치.
  - ⚡ 자동정렬 시 상하(TD) 및 좌우(LR) 완벽 격자 재배치.
- **초슬림 마이크로 컴팩트 UI 및 폰트 최적화**:
  - 하단 팔레트 버튼 높이 28dp, 폰트 10sp로 축소하여 모바일 캔버스 작업 공간 대폭 확장.
  - 노드 폰트 9sp로 컴팩트화하여 가독성과 공간 효율 극대화.
- **최신 APK 빌드 완료**: `ManualStudioMobile.apk` 배포본 갱신.
