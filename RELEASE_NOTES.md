...
- **플로우차트 자동정렬(상하 TD / 좌우 LR) 및 캔버스 오버플로우 방지 엔진**:
  - 캔버스 내 흩어진 플로우차트 도형 노드들을 토폴로지 연결 순서에 따라 자동 랭킹/계층화.
  - 상하 수직 정렬(`TD`)과 좌우 수평 정렬(`LR`)을 원클릭으로 상호 전환.
  - 캔버스 작업 영역을 벗어나지 않도록 단계별 여백 클램핑 및 동적 스케일링 적용.
  - 노드 간 자석 연결선(직선/직각)도 실시간으로 경로 재계산 및 정렬 추종.
