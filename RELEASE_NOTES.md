...
### ⚡ 플로우차트 지능형 자동정렬 (연결 노드 위상 정렬 & 독립 노드 바둑판식 배열)
- **연결선(화살표) 유무에 따른 노드 지능형 이원화 처리**:
  - **연결 노드 (Connected Nodes)**: 상하(TD) 및 좌우(LR) 위상 계층(Topological BFS) 순서로 자동 배치 및 최적 마그넷 포인트(상하좌우) 연결선 자동 재부착.
  - **미연결 독립 노드 (Unconnected Nodes)**: 아직 배치 단계가 미정인 후보 프로세스로 인식하여, 메인 플로우차트에 강제 삽입하지 않고 Windows 바탕화면 아이콘 정렬 스타일의 **바둑판식(Grid) 배열**로 우측/하단 대기 영역에 정렬.
- **전원 독립 노드 지원**: 연결선이 전혀 없는 상태에서도 모든 노드를 겹침 없는 균등 격자(바둑판) 형태로 일괄 자동정렬.
- **연속 클릭 시 TD ↔ LR 피벗 토글**: 동일 상태에서 재클릭 시 상하(TD)와 좌우(LR) 방향이 자동 전환되며 독립 노드 도킹 위치도 동적으로 연동.

### 🧭 플로우차트 지능형 장애물 회피 연결선 라우팅 (1번/2번 꺾임 우회)
- **노드 충돌 검사 및 회피 경로 자동 산출 (`FlowchartRoutingEngine`)**:
  - 연결선 벡터 경로 상에 존재하는 다른 다이어그램(노드) 장애물 검출.
  - 충돌 미발생 시 1번 꺾임(HV/VH 직각 지선) 최우선 적용.
  - 경로 상에 중간 노드가 존재할 경우 2번 꺾임(VHV/HVH 및 장애물 외곽 우회 채널)으로 다른 다이어그램을 침범하지 않고 자동 우회.
- **`ElbowArrowItem` 다점 꺾임 지원 (`custom_corners`)**:
  - 1번 꺾임 및 2번 꺾임의 임의 중간 지선 좌표 지원, 렌더링, 히트테스트, `.drg` / `.mcs.json` 직렬화 100% 보존.
- **실시간 반응형 연동**:
  - 노드 간 마우스 드래그 연결, 노드 위치 이동 시 동적 추종, 자동정렬 재부착 파이프라인 전 구간에 지능형 우회 라우팅 통합 적용.

## [v1.9.9] - 2026-09-19

### 🏛️ 라이선스 4대 표준 분류 체계 개편 & 폐쇄망 옵션 배제
- **4대 표준 라이선스 모델 정립**:
  - ① **기업용 (`ENTERPRISE` - `MSENT`)**: 일반 기업, 금융사, **공기업, 준정부기관** 포함.
  - ② **관공서용 (`GOVERNMENT` - `MSGOV`)**: 중앙행정기관, 국회·법원, 광역·기초 지방자치단체 등 순수 행정 관공서 전용.
  - ③ **교육용 (`EDUCATION` - `MSED`)**: 초·중·고교, 대학교, 대학원, 직업학원 및 학생·교직원 전용.
  - ④ **개인용 (`PERSONAL` - `MSPS`)**: 1인 개발자, 프리랜서 전용 1카피 라이선스.
- **공기업 분류 기준 통일**: 공기업 및 공공기관은 기업용(`ENTERPRISE`)으로 일원화 분류.
- **폐쇄망 옵션 배제**: 복잡성 및 웹앱 연동 한계로 폐쇄망(`AIR_GAPPED`)은 라이선스 분류에서 전면 제외.
- **하위 호환성 유지**: 기존 영구(`MS1P`), 1개월(`MS1M`), 1년(`MS1Y`), 평가 연장(`MST14`) 시리얼 키도 정상 인증 완벽 지원.

### 🎨 기업/교육/관공서 전용 커스텀 워터마크 지정 기능 (`WatermarkConfigDialog`)
- **권한 기반 제어**: 기업용(`ENTERPRISE`), 관공서용(`GOVERNMENT`), 교육용(`EDUCATION`) 라이선스 인증 시 기능 활성화.
- **전사 표준 UI/UX 준수**: 무수식어 건조한 명사 레이블 및 상하 세로 스택(`flex-direction: column`, `gap: 4px`) 배치.
- **7대 출력 위치 지원**: 중앙 대각선, 중앙 수평, 우측 하단, 좌측 하단, 우측 상단, 좌측 상단, 바둑판 타일링.
- **자유로운 속성 조절**:
  - 문구 입력, 글꼴 크기(12~120pt), 회전 각도(-90°~+90°), 투명도(5%~100%), 색상 팔레트, 굵기 토글.
- **실시간 반응형 미리보기**: 조작 즉시 우측 캔버스에서 가상 매뉴얼 문서와 결합된 워터마크 렌더링 결과 실시간 표출.
- **종단간 렌더링 파이프라인 연동**: 캔버스 합성, 이미지 내보내기, 슬라이드/PPT/한글 전송 시 워터마크 자동 합성.
